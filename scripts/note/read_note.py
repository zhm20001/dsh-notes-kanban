...
folder) and prints the note's raw markdown document so the host agent can
render it. A missing note exits non-zero with a message on stderr and nothing
on stdout.

Usage::

    read_note.py --notes-dir DIR --id FILE

(Structured ``{front_matter, body}`` output is deferred to ticket 02, where
integration actually needs to read a note programmatically.)
"""
